...
class LLMEvaluatorAgent:
    """Acts as an objective quantitative QA judge using Pydantic validation and think-tag filtering."""
...
- Do NOT include any markdown code blocks (like ```json), backticks, or conversational text.
  ...
  "verdict": "string"
        ...
        try:
            response = await self.client.chat.completions.create(
                ...
                temperature=0.1,
                max_tokens=1500,  # Retains headroom for reasoning overhead
            )
            raw_content = response.choices[0].message.content
            
            # Strip out <think>...</think> reasoning blocks cleanly
            cleaned_content = re.sub(r"<think>.*?</think>", "", raw_content, flags=re.DOTALL)
            
            # Extract JSON substring from the remaining text
            ...
            # Parse to dict first, then validate strictly via Pydantic
            ...
            return {
                ...
                "verdict": "Evaluation failed due to schema validation or API error."
            }