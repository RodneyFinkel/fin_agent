...
class MarketDataPipelineTool:
    ...
    description: str = (
        ...
        "Yahoo's direct chart API using curl_cffi chrome impersonation to bypass rate limits and bot detection."
    )
    ...
    def _fetch_historical_yfinance_safely(self, ticker):
        """Fetches historical data directly from Yahoo's chart API using curl_cffi impersonation, bypassing yfinance wrapper bugs."""
        try:
            session = cffi_requests.Session(impersonate="chrome")
            # period1=1546300800 corresponds to Jan 1, 2019
            url = f"https://query1.finance.yahoo.com/v8/finance/chart/{ticker}?period1=1546300800&period2=9999999999&interval=1d"
            response = session.get(url)
            
            if response.status_code != 200:
                print(f"API Error for {ticker}: Status code {response.status_code}")
                return None, 'Unknown', 'Unknown'
            
            data = response.json()
            result_array = data.get('chart', {}).get('result')
            if not result_array:
                print(f"No chart result found for ticker: {ticker}")
                return None, 'Unknown', 'Unknown'
            ...
            if not timestamps or not quotes:
                print(f"Missing price or timestamp data for ticker: {ticker}")
                return None, 'Unknown', 'Unknown'
            ...
            if df.empty:
                return None, company, sector
                
            df.index = df.index.tz_localize(None)
            
            return df, company, sector
        except Exception as e:
    ...
    def run(self, tickers: list[str], parquet_path: str = "stock_cache.parquet", sqlite_path: str = "stock_database.db") -> str:
        ...
        for idx, ticker in enumerate(tickers_to_fetch):
            if idx > 0:
                print(f"[Pipeline] Applying rate-limit delay...")
                time.sleep(2)
